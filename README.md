...
To determine the number of main chain elements that need to be considered for inserting a pending element, the formula `2^k - 1` is used<sup><a href="#footnote1">[1]</a>,<a href="#footnote2">[2]</a></sup>, where `k` is the maximum number of comparisons allowed for an element in that insertion group. This value defines the "useful portion" of the main chain for binary search. By constraining the search space to at most `2^k - 1` elements, the algorithm guarantees that any element from this group can be inserted using no more than `k` comparisons, thus adhering to the optimal procedure.
